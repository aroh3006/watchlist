...
Watchlist is a personal TV-show and movie tracking app — a modern, original successor to the
...
2. **Extract** — ZIP entries are read into memory only (nothing touches disk), and any entry with
...
5. **Match** — priority order: exact external ID (imdb/tmdb/tvdb via `ExternalId`) → exact
   normalized title → fuzzy title → ambiguous (multiple candidates, never auto-picked) →
   unmatched.
...
7. **Commit** — matched/resolved rows become `EpisodeWatch`/`MovieWatch`/`Rating`/`Favorite`
   ...
   so **re-running the same import is always safe** — it upserts, never duplicates.
...
This is designed as a **TV Time-export-compatible generic CSV/ZIP importer** — because TV Time's
exact export schema isn't known/available here, the importer works from real column names rather
